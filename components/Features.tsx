    ...
    <section id="features" className="py-24 sm:py-32" aria-labelledby="features-title">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-none">
          <dl className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-16 lg:max-w-none lg:grid-cols-4" role="list">
            {features.map((feature) => (
              <div key={feature.name} className="flex flex-col" role="listitem">
                <dt className="text-base font-semibold leading-7 text-gray-900">
                  <div
                    className="mb-4 flex h-16 w-16 items-center justify-center rounded-lg bg-indigo-600 text-3xl"
                    aria-label={`Icono de ${feature.name}`}
                    role="img"
                  >
                    {feature.icon}
                  </div>
